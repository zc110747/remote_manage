...
using TIMER_STRUCT_V2::timer_struct;
...
public:
    ...
    timer_action(int id, uint32_t time, uint32_t count, std::function<void(int)> func):
        event_id_(id), 
        tick_compare_(time), 
        action_times_(count),
        time_handler_(func) 
...
private:
    ...
    std::vector<timer_action> time_action_vec_;
