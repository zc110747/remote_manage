...
void device_manage::process_hardware(Event *pEvent)
{
    ...
    PRINT_LOG(LOG_INFO, xGetCurrentTimes(), "Device Process:%d, %d!", device, action);
    ...
    case EVENT_DEVICE_BEEP:
        ...
        break;
    default:
        PRINT_LOG(LOG_ERROR, xGetCurrentTimes(), "Invalid Device:%d!", device);
        break;
    }
}
...
void device_manage::run()
{
    ...
    for (;;)
    {
        size = device_fifo_point_->read(buffer, RX_BUFFER_SIZE);
        if (size > 0)
        {
            PRINT_LOG(LOG_DEBUG, xGetCurrentTimes(), "Device Command, size:%d, id:%d!", 
                size,
