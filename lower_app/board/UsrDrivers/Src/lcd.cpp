...
void lcd_driver::config_init(void)
{
		//read device id
		lcd_wr_reg(0XDA00);	
		id_ = lcd_rd_data();		//读回0X00	 
		lcd_wr_reg(0XDB00);	
		id_ = lcd_rd_data();		//读回0X80
		id_<<=8;	
		lcd_wr_reg(0XDC00);	
		id_ |= lcd_rd_data();	//读回0X00		
...
void lcd_driver::lcd_scan_dir(uint8_t dir)
{
	...
	//if(id==0x9341||id==0X5310||id==0X5510||id==0X1963)//9341/5310/5510/1963,特殊处理
	{
		switch(dir)
		{
...
void lcd_driver::lcd_clear(uint32_t color)
{
	...
	totalpoint *= height_; 						//得到总点数
	lcd_setcursor(0x00, 0x0000);			//设置光标位置 
	write_ram_prepare();     			//开始写入GRAM	 	  
...
void lcd_driver::test()
{
	...
	lcd_showstring(10, 150,240, 12, 12, (char *)"2016/1/6");	  
}