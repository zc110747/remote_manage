...
static int imx_init_from_nvmem_cells(struct platform_device *pdev)
{
	...
	if (ret)
	{
		dev_err(&pdev->dev, "nvmem_cell_read_u32 calib err!\n");
		return ret;
	}
	...
	if (ret)
	{
		dev_err(&pdev->dev, "imx_init_calib err!\n");
		return ret;
	}
	...
	if (ret)
	{
		dev_err(&pdev->dev, "nvmem_cell_read_u32 temp_grade err!\n");
		return ret;
	}
	imx_init_temp_grade(pdev, val);
...
static int imx_thermal_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s run 1\n", __func__);
	data = devm_kzalloc(&pdev->dev, sizeof(*data), GFP_KERNEL);
	if (!data)
	{
		dev_err(&pdev->dev, "devm alloc failed\n");
		return -ENOMEM;
	}

	data->dev = &pdev->dev;

	/* 获取节点匹配的map属性，赋值data->tempmon */
	...
	pm_runtime_put(data->dev);
	dev_info(&pdev->dev, "%s run success\n", __func__);
