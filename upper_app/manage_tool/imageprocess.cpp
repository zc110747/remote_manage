...
QImage COpencvImgProcess::cvMattoQImage(const cv::Mat& mat)
{
    switch(mat.type())
    {
        ...
        case CV_8UC3:
        {
            QImage image(mat.data, mat.cols, mat.rows, (int)mat.step, QImage::Format_RGB888);	// R, G, B 对应 0,1,2
        ...
        case CV_8UC4:
        {
            QImage image(mat.data, mat.cols, mat.rows, (int)mat.step, QImage::Format_ARGB32);		// B,G,R,A 对应 0,1,2,3
            return image;
        }
        break;
        default:
...
bool COpencvImgProcess::blur_image(QLabel *label, QString Path)
{
    ...
    cv::blur(ImgMat, ImgMatOut, cv::Size(7, 7));                    //均值滤波
    //cv::GaussianBlur(ImgMat, ImgMatOut, cv::Size(7, 7), 0, 0);    //高斯滤波
    //cv::medianBlur(ImgMat, ImgMatOut, 3);                         //中值滤波
    //cv::bilateralFilter(ImgMat, ImgMatOut, 4, 4*2, 4/2);            //双边平滑
